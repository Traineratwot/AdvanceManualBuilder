dataTypes.toArray = (arr = {}) => {
	for (let element in dataTypes) {
		...
		if (e instanceof CLASSES.DataTypeClass) {
			if (e.subName == e.name) {
				arr[element] = (`${locale._(e.subName)}`)
			} else {
				arr[element] = (`${locale._(e.subName)} (${e.name})`)
			}
		}
	}
	return arr
...
dataTypes.int = new CLASSES.DataTypeClass({ name: 'int', subName: 'int', preview: '1' })
dataTypes.boolean = new CLASSES.DataTypeClass({ name: 'boolean', subName: 'boolean', preview: 'true' })
dataTypes.float = new CLASSES.DataTypeClass({ name: 'float', subName: 'float', preview: '0.5' })
...
dataTypes.timestamp = new CLASSES.DataTypeClass({ name: 'int', subName: 'timestamp', preview: '1601647148' })
